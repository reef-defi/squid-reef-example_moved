...
You can visit [Subsquid docs](https://docs.subsquid.io/) for more information.
...
It is necessary to import the respective ABI definition to decode EVM logs. For REEF transfers we use the standard ERC20 interface, see [`src/abis/ERC20.json`](src/abis/ERC20.json).

To generate a type-safe facade class to decode EVM logs, use `squid-evm-typegen(1)`:

```bash
npx squid-evm-typegen src/abi src/abi/ERC20.json
```
